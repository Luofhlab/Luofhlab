...
readtre <-function(tre,cor,hmp,design=FALSE,de_leve="none",de="none",short=0){
		library(ape)
		source("/mnt/ilustre/users/huimin.zhang/newmdt/scripts/plot.phylo.r")
		#ord_row <- c()
		ord_lab <-c()
...
write.table(otu_hmp_tmp,row.names=FALSE,"heatmap_species_rtop50.pdf.xls",sep="	",eol="
",quote=FALSE)
#library(gplots)
kbn <-200
brks <-vector()
...
pdf0 <-"heatmap_species_rtop50.pdf"
