...
otu <-read.table(file="species.xls",header=T,check.names=FALSE,sep="\t",fill=T)
rownames(otu) <- otu[,1]
...
write.table(otu_hmp_tmp,row.names=FALSE,"heatmap_species.pdf.xls",sep="	",eol="
",quote=FALSE)
#library(gplots)
kbn <-200
brks <-vector()
...
pdf0 <-"heatmap_species.pdf"
pdf(pdf0,width=10,height=10)
...
if(marble[1]>0) mtext(collab,side =1,at=c(1:nc),cex=0.8,las=clas,line=0.7) 
if(marble[2]>0) mtext(rowlab,side =2,at=c(1:nr),cex=0.6,las=1,line=0.5,font=3) 
if(marble[3]>0) mtext(collab,side =3,at=c(1:nc),cex=0.8,las=clas,line=0.7) 
...
image(c(0:kbn),0:1,z=matrix(c(1:kbn),kbn,1),xlim=c(0,kbn),col=heatcol,yaxt="n",xlab=xlab,ylab="",axes=FALSE,add=FALSE,cex.lab=0.8*1.5)
labl <-brks[at+1]
if(logo=="T"){
...
axis(side=1,at=at[-(length(at))],labels=labl[-(length(at))],pos=c(0,0),cex.axis=0.8*1.5)
axis(side=1,at=kbn,labels=round(mo,digits=dig),pos=c(0,0),cex.axis=0.8*1.5)
